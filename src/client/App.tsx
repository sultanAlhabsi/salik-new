...
const demos = [
  { label: 'Admin demo', email: 'admin@salik.om', icon: Building2, detail: 'Platform operations' },
  { label: 'Supplier demo', email: 'supplier@fresh.om', icon: Warehouse, detail: 'Catalog and fulfillment' },
  { label: 'Store demo', email: 'store@alnoor.om', icon: Store, detail: 'Procurement and invoices' },
  { label: 'Driver demo', email: 'driver@fresh.om', icon: Truck, detail: 'Routes and delivery proof' }
];
...
function LoginScreen({ onLogin }: { onLogin: (user: CurrentUser) => void }) {
  ...
  async function authenticate(loginEmail = email) {
    setBusy(true); setError('');
    try { const result = await login(loginEmail, password); onLogin(result.user); } catch (err) { setError(err instanceof Error ? err.message : 'Sign in failed'); } finally { setBusy(false); }
  }
